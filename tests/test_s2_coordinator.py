...
@pytest.mark.asyncio
async def test_cem_frbc():  # TODO: move into different test functions
    cem = CEM(sensor_id=1, fm_client=None)
    frbc = FRBCTest()
    ppbc = PPBC()
    cem.register_control_type(frbc)
    cem.register_control_type(ppbc)
    # Show that this point is reached
...
# @pytest.mark.asyncio
# async def test_cem_ppbc():  # TODO: move into different test functions
#     cem = CEM(sensor_id=1, fm_client=None)
#     ppbc = PPBC()
#     cem.register_control_type(ppbc)
#     # Show that this point is reached
